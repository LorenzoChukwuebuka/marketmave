...
@section('content')
    <div class="col-xl-9">
        ...
        <div class="mb-4 text-end">
            <button class="btn btn--primary {{ $wallets->count() < $general->wallet_limit ? 'confirm' : 'sorry' }} h-auto">
                @lang('Add New Wallet')
            </button>
        </div>
                ...
                <tbody>
                    @forelse($wallets as $data)
                        ...
                        <tr>
                            ...
                            <td data-label="@lang('Wallet Address')"><span
                                    id="{{ $data->id }}">{{ $data->wallet_address }}</span> <a href="javascript:void(0)"
                                    class="badge text-white badge--primary copytext" data-id="{{ $data->id }}">copy </a>
                            </td>
                            <td data-label="@lang('Balance')">{{ showAmount($data->balance, 8) }}
                                {{ __($general->cur_text) }}</td>
                            <td data-label="@lang('Action')">
                                <button class="btn btn-sm btn--primary qrcode h-auto"
                                    data-wallet="{{ $data->wallet_address }}">@lang('Receive')</button>
                            </td>
                        </tr>
                        <tr class="d-block">
                            <td class="d-none"></td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="{{ route('user.add.wallet') }}">
                    ...
                    <div class="modal-body text-center">
                        <strong class="text-dark">@lang('Do you want to give your wallet name and address?')</strong>
                        <input type="text" name="name" class="form-control mt-2" placeholder="Wallet Name" required>
                        <input type="text" name="wallet_address" class="form-control mt-2" placeholder="Wallet Address">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn--danger cmn--btn btn--sm"
                            data-bs-dismiss="modal">@lang('Close')</button>
                        <button type="submit" class="btn btn--success cmn--btn btn--sm">@lang('Confirm')</button>
                    </div>
                </form>
            </div>
        </div>
